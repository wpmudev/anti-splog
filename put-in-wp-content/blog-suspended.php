...
<?php if (!get_option('ust_email_sent')) { ?>
  ...
  <p>
		...
		<textarea name="reason" style="width: 100%" rows="5" tabindex="20"><?php echo esc_textarea($reason); ?></textarea></label>
	</p>
	<?php
    $recaptcha = get_site_option('ust_recaptcha');
  ...
  ?>
	<br class="clear" />
	<p class="submit"><input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php _e('Submit', 'ust'); ?>" /></p>
<?php } else { ?>
